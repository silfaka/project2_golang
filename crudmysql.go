...
//sintax -> sql.Open("mysql", "user:password@tcp(host:port)/Task_detail_database")
...
func tampil(pesane string) response {
	...
	for dataMhs.Next() {
		var mhs = mahasiswa{}
		var salahe = dataMhs.Scan(&mhs.Nim, &mhs.Task_detail, &mhs.Assignee, &mhs.Deadline)
...
//fungsi tambah data
func tambah(nim string, Task_detail string, Assignee string, Deadline string) response {
	db, salahe := koneksi()
	if salahe != nil {
		return response{
			Status: false,
	...
	_, salahe = db.Exec("insert into mahasiswa values (?, ?, ?, ?)", nim, Task_detail, Assignee, Deadline)
	if salahe != nil {
		return response{
			Status: false,
...
//fungsi ubah data
func ubah(nim string, Task_detail string, Assignee string, Deadline string) response {
	db, salahe := koneksi()
	if salahe != nil {
		return response{
			Status: false,
	...
	_, salahe = db.Exec("update mahasiswa set Task_detail=?, Assignee=?, Deadline=? where nim=?", Task_detail, Assignee, Deadline, nim)
	if salahe != nil {
		return response{
			Status: false,
...
func kontroler(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		var Task_detail = r.FormValue("Task_detail")
		var Assignee = r.FormValue("Assignee")
		var Deadline = r.FormValue("Deadline")

		var aksi = r.URL.Path
		if aksi == "/tambah" {
			var hasil = tambah(nim, Task_detail, Assignee, Deadline)
			tampilHtml.Execute(w, tampil(hasil.Pesan))

		} else if aksi == "/ubah" {
			var hasil = ubah(nim, Task_detail, Assignee, Deadline)
			tampilHtml.Execute(w, tampil(hasil.Pesan))
		} else if aksi == "/hapus" {
			var hasil = hapus(nim)
			tampilHtml.Execute(w, tampil(hasil.Pesan))
